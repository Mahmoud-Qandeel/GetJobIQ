...
The salary lookup tool (`01-FRAMEWORK/salary_lookup.py`) lets you benchmark company salaries against a baseline from your own data. It's used during the `/apply` workflow to show how a company's compensation compares to market rates.
...
```bash
python3 01-FRAMEWORK/salary_lookup.py "Novo Nordisk"
python3 01-FRAMEWORK/salary_lookup.py "Ørsted" --city "Fredericia"
python3 01-FRAMEWORK/salary_lookup.py "COWI" --json
python3 01-FRAMEWORK/salary_lookup.py --list-all
python3 01-FRAMEWORK/salary_lookup.py --validate      # pre-flight check your salary_data.json
```

## Important notes

- The data file (`salary_data.json`) is **excluded from git** (see `.gitignore`). Your salary data may be proprietary or confidential.
- If the data file is missing, `01-FRAMEWORK/salary_lookup.py` exits with a helpful error message and the `/apply` workflow skips the salary benchmark step.
