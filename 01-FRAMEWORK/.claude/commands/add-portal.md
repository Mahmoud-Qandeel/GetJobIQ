...
- If `$ARGUMENTS` contains `--list`: use Glob with `01-FRAMEWORK/.agents/skills/*/SKILL.md`, print a table of installed portal skills (name, market from the description, data source from `url-reference.md`), and stop.
...
2. **Skill name** - kebab-case, suffixed `-search` (e.g. `seek-search`, `stepstone-search`). Must not collide with an existing folder in `01-FRAMEWORK/.agents/skills/`.
...
**Canonical reference:** read `01-FRAMEWORK/.agents/skills/linkedin-search/` before generating - it is the zero-dependency worked example of this exact structure. Copy its architecture, not its LinkedIn-specific parsing.

Create `01-FRAMEWORK/.agents/skills/<name>/` with:
...
1. Install dev types and typecheck:
   ```bash
   cd 01-FRAMEWORK/.agents/skills/<name>/cli && bun install && bun run typecheck
   ```
...
1. Ask whether the user wants the new portal added to their `/scrape` search strategy. If yes:
   - The portal CLI itself is already picked up automatically by `/scrape` (it discovers `01-FRAMEWORK/.agents/skills/*/SKILL.md`) — no further wiring is needed for CLI search/detail.
   - Optionally add WebSearch/`site:` placeholder queries for that board in `01-FRAMEWORK/.claude/skills/job-scraper/search-queries.md` (use the `[YOUR_JOB_BOARD]` style placeholders already there) so the fallback path still covers the board if the CLI is unavailable.
2. Remind the user to add the install line for their own records if they maintain a fork README:
   ```bash
   cd 01-FRAMEWORK/.agents/skills/<name>/cli && bun install && cd ../../../..
   ```
...
4. CI coverage is also automatic: the `cli-checks` job discovers every `01-FRAMEWORK/.agents/skills/*/cli/package.json`, so the new CLI's `typecheck` and `test` scripts run on every push to the fork without editing the workflow.
...
>
> - Files: `01-FRAMEWORK/.agents/skills/<name>/` (SKILL.md, url-reference.md, CLI with tests)
...
>
> Try it: `bun run 01-FRAMEWORK/.agents/skills/<name>/cli/src/cli.ts search -q "<test query>" --format table`
>
