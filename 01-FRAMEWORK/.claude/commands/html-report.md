...
Generate a self-contained HTML dashboard from `job_search_tracker.csv` and the application archives under `02-Documents/applications/`. The output is a single `.html` file — no server, no dependencies — that can be opened directly in a browser.
...
- **Footer:** "Generated by Claude Code · GetJobIQ · {ISO date}"
