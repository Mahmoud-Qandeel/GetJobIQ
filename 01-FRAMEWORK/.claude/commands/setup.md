...
You are running the onboarding setup for the GetJobIQ framework. Your goal is to collect the user's professional information and populate all profile files so the `/apply` workflow works out of the box.
...
Then, before greeting the user, scan the `02-Documents/` folder. Use Glob with `02-Documents/**/*` and count files per subfolder (`02-Documents/cv/`, `02-Documents/linkedin/`, `02-Documents/diplomas/`, `02-Documents/references/`, `02-Documents/applications/`).

Then welcome the user with a single message that lists three paths. The wording changes based on what was found.

**If `02-Documents/` has files** in one or more subfolders, lead with Path A:

> **Welcome to the GetJobIQ setup!**
>
...
>
> I see files in your `02-Documents/` folder: [list per subfolder, e.g. "2 in 02-Documents/cv/, 1 in 02-Documents/linkedin/, 3 in 02-Documents/references/"]. Three ways to start:
>
> **Path A: Read my documents folder** (recommended for what you have) - I'll read everything in `02-Documents/`, cross-reference for consistency, and build your profile from real source materials. Idempotent and safe to re-run as you add more documents.
>
...
**If `02-Documents/` is empty or missing**, surface Path A as a "do this if you have materials" option:
...
> Three ways to start:
>
> **Path A: Documents folder** (best signal if you have several materials) - Drop your CV / LinkedIn export / diplomas / reference letters in the `02-Documents/` folder, then say "go". I'll read everything and build your profile from it. See `02-Documents/README.md` for the folder layout.
>
> **Path B: Single CV import** - Paste or @-mention a single CV/resume here. I'll extract it and ask follow-up questions for what's missing.
>
...
Wait for the user's choice. If they pick A but the folder is still empty, tell them what to add (point at `02-Documents/README.md`) and stop.

---

## Path A: Documents Folder

Reads structured documents in `02-Documents/`, cross-references them for consistency, and merges extracted data into the seven profile skill files. Read-before-write and idempotent: changes already present will not be proposed again.

Follow these steps **exactly in order**.

### Step A1: Inventory

Use Glob with `02-Documents/**/*` to scan the full tree. Print:

```
## Documents Found

**02-Documents/cv/**: [list files, or "(empty)"]
...
If every subfolder is empty, stop and tell the user to populate the folder. Point at `02-Documents/README.md` for the layout.
...
- `01-FRAMEWORK/.claude/skills/job-application-assistant/03-writing-style.md`
...
Read each document found in Step A1. Process subfolders in this order: `02-Documents/cv/`, `02-Documents/linkedin/`, `02-Documents/diplomas/`, `02-Documents/references/`, `02-Documents/applications/`.

**`02-Documents/cv/` documents:** name, contact (email, phone, LinkedIn, GitHub), education (degree, institution, dates, thesis), work experience (title, company, dates, location, bullets), skills, languages (with any stated proficiency), publications, awards, profile/summary.
...
- Languages you work in professionally, with proficiency levels (only if not already extracted from `02-Documents/cv/` or `02-Documents/linkedin/` above) - this feeds the Language Gate in `04-job-evaluation.md`, so ask directly rather than skipping it
...
Then proceed to Step 3 to populate the non-skill files (`01-FRAMEWORK/CLAUDE.md`, `04-APPLICATIONS/cv/main_example.tex`, `01-FRAMEWORK/.claude/skills/job-scraper/search-queries.md`). Step 3 will detect that the seven skill files are already populated and skip those substeps.
...
- **Job portals:** "The framework ships country-agnostic search CLIs (`linkedin-search`, `freehire-search`, enabled by default) plus Danish portal demos (Jobindex, Jobbank, Jobdanmark, Jobnet) that ship **disabled**. `/scrape` auto-discovers whatever portal skills are installed under `01-FRAMEWORK/.agents/skills/` and skips any with `enabled: false`. Which portals fit your market?" **Then act on the answer:** if the user's market is Denmark (or they ask for the Danish boards), edit each of the four Danish `SKILL.md` files and set `enabled: true` in the frontmatter; otherwise leave them disabled and say so - they cost nothing while disabled and can be enabled later by flipping the flag. If the user needs a local board that is not shipped, guide them to `/add-portal` (market-specific skills live in their fork). WebSearch/`site:` queries remain the fallback for portals without a CLI skill.
- **CV language:** "Should your CVs be written in English (the default, accepted in most markets), or in your market's language?" Record the answer as a `CV language: <language>` line in 01-FRAMEWORK/CLAUDE.md's Identity section. Cover letters always match each posting's language automatically; this setting governs the CV only. If the user is unsure, keep English and note they can re-run `/setup --section search` to change it.
...
### 1. Update `01-FRAMEWORK/CLAUDE.md`
...
>
> - `01-FRAMEWORK/CLAUDE.md` - Your full candidate profile
> - `01-FRAMEWORK/.claude/skills/job-application-assistant/01-candidate-profile.md` - Structured profile
> - `01-FRAMEWORK/.claude/skills/job-application-assistant/02-behavioral-profile.md` - Behavioral assessment
> - `01-FRAMEWORK/.claude/skills/job-application-assistant/04-job-evaluation.md` - Personalized evaluation framework
> - `01-FRAMEWORK/.claude/skills/job-application-assistant/05-cv-templates.md` - CV templates with your profile statements
> - `01-FRAMEWORK/.claude/skills/job-application-assistant/07-interview-prep.md` - STAR examples from your experience
> - `04-APPLICATIONS/cv/main_example.tex` - Your LaTeX CV template
> - `01-FRAMEWORK/.claude/skills/job-scraper/search-queries.md` - Job search queries for `/scrape`
>
...
- Three onboarding paths converge on the same skill files. Step 0 picks the right path based on what's in `02-Documents/`. Steps 3 and 4 are shared.
