...
**⚠️ Static Snapshot:** This file is generated by `/setup` as a one-time snapshot of your target roles, domains, and skills from your profile. If you update your target roles or focus areas significantly (via `/setup --section search` or manual edits to `01-FRAMEWORK/CLAUDE.md`), regenerate this file by running `/setup` again — otherwise new search coverage will not be reflected in `/scrape` runs. The queries below reflect your profile as of generation date.
