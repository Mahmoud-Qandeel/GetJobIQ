...
allowed-tools: Bash(bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts *)
---
...
```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts search [flags]
```
...
```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts detail <slug> [--format json|plain]
```
...
```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts categories [--format json|table|plain]
```
...
```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts autocomplete --query "<text>" [--limit <n>]
```
...
```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts locations --query "<text>" [--limit <n>]
```
...
```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts locations --query "Aarhus" --format plain
```

**Resolve job titles for precision.** Use `autocomplete` to get the exact job title ID when the user wants a specific role:

```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts autocomplete --query "sygeplejerske" --format plain
```
...
```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts search \
  --category 227978 \
...
```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts categories --format table
```

### Full details for a specific job posting

```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts detail it-chef-soeges-til-rah --format plain
```

### Find jobs in zip code 8000

```bash
bun run 01-FRAMEWORK/.agents/skills/jobdanmark-search/cli/src/cli.ts search --zip 8000 --format table
```
